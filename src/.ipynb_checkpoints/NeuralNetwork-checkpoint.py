...
class CudaLayer:
    ...
    def initialize(self, arr, low=-1, high=1, mode=None):
        ...
        sampler = getattr(np.random, mode)
        ...
        self.history['w_init'] = np.linalg.norm(self.weights).get()
    ...
    def backward(self, error, lr):
        self.history['w'].append(np.linalg.norm(self.weights).get())

        self.error.append(error)

        # Compute gradients
        backward = np.multiply(self.logs['dact'], error)
        new_weights = np.matmul(self.logs['X'].transpose(), backward)
        new_bias = np.mean(backward, axis=0)

        # Compute aux term for next layer
        backward = np.matmul(backward, self.weights.transpose())
...
class CudaNetwork:
    ...
    def __init__(self, poly_reg=False, round_off=30):
    ...
    def _single_fit(self, X, y, lr):
        ...
        return np.sum(self.__loss_function__.compute(X=predictions, y=y, dy=False))
    ...
    def forward(self, X):
        ...
        if self.poly_reg:
            X = np.power(X, np.arange(self.poly_reg))
    ...
    def predict(self, X):
        return np.round(self.forward(X=X), self.__round_off__)
    ...
    def decode_weights(self, weights):
        for layer, weight in zip(self.layers, weights):
            layer.weights = np.array(weight[0]).reshape(-1, weight[-1])
...
if __name__ == '__main__':
    def experiment(mode):
        ...
        x = np.arange(11)
        y = (x ** 2).reshape(-1, 1)
        x = np.vstack([np.zeros_like(x), x]).transpose()
        tmp = l.forward(X=x)
        return tmp
