...
class Config:
    hyperParameter = {
        ...
        'optimizer': {
            'momentum': {
                'initial_momentum': lambda x, y: kernel.kernel.ones(shape=(x, y)),
                'beta': 9e-1,
            ...
            'adaGrad': {
                'initial_gradient': lambda x, y: kernel.kernel.zeros(shape=(x, y)),
                'learning_rate_decay': 0.9999
            },
            ...
            'adam': {
                'initial_gradient': lambda x, y: kernel.kernel.zeros(shape=(x, y)),
                'initial_momentum': lambda x, y: kernel.kernel.zeros(shape=(x, y)),
                'momentum_beta': 2e-10,
                'gradient_beta': 4e-3,
            }
        },
